...
import com.example.bottomnavbar.databinding.PharmacyFragmentDetailsBinding;
import com.google.android.material.appbar.CollapsingToolbarLayout;
import com.squareup.picasso.Picasso;

import java.util.ArrayList;

public class DetailsPharmacyFragment extends Fragment  {
    ...
    public DetailsPharmacyFragment() {
        // Required empty public constructor
    }


    public static DetailsPharmacyFragment newInstance(Pharmacy pharmacy) {
        DetailsPharmacyFragment fragment = new DetailsPharmacyFragment();
        Bundle args = new Bundle();
        args.putSerializable(ARG_PARAM_CONTACT, pharmacy);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            mPharmacy = (Pharmacy) getArguments().getSerializable(ARG_PARAM_CONTACT);
        }
    }

    PharmacyFragmentDetailsBinding binding;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = PharmacyFragmentDetailsBinding.inflate(inflater, container, false);

        view = inflater.inflate(R.layout.pharmacy_fragment_details, container, false);
        ...
        uber.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String uri = "https://m.uber.com/?client_id=<mHClirme28wuZEzqPiTuviNhVKd-FTtE>&dropoff[latitude]=" + mPharmacy.getLatitude() + "&dropoff[longitude]=" + mPharmacy.getLongitude();
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(uri));
                getContext().startActivity(intent);
            }
        });
        ...
        cvMaps.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String uri = "http://maps.google.com/maps?q=loc:" + mPharmacy.getLatitude() + "," + mPharmacy.getLongitude() + " (" + mPharmacy.getName() + ")";
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(uri));
                getContext().startActivity(intent);
        ...
        placename.setText(mPharmacy.getName());
        distance.setText(mPharmacy.getDistance());
        mapdis.setText(mPharmacy.getDistance());
        String url = "https://foursquare.com/v/" + mPharmacy.getFrq_id();
        description.setText(url);

        Picasso.get().load(mPharmacy.getImage()).into(ivbackcoll);
