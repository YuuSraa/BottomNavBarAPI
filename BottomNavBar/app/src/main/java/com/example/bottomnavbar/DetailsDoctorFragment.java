...
import com.example.bottomnavbar.databinding.DoctorFragmentDetailsBinding;
import com.google.android.material.appbar.CollapsingToolbarLayout;
import com.squareup.picasso.Picasso;

import java.util.ArrayList;

public class DetailsDoctorFragment extends Fragment  {
    ...
    public DetailsDoctorFragment() {
        // Required empty public constructor
    }


    public static DetailsDoctorFragment newInstance(Doctor doctor) {
        DetailsDoctorFragment fragment = new DetailsDoctorFragment();
        Bundle args = new Bundle();
        args.putSerializable(ARG_PARAM_CONTACT, doctor);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            mDoctor = (Doctor) getArguments().getSerializable(ARG_PARAM_CONTACT);
        }
    }

    DoctorFragmentDetailsBinding binding;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = DoctorFragmentDetailsBinding.inflate(inflater, container, false);

        view = inflater.inflate(R.layout.doctor_fragment_details, container, false);
        ...
        uber.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String uri = "https://m.uber.com/?client_id=<mHClirme28wuZEzqPiTuviNhVKd-FTtE>&dropoff[latitude]=" + mDoctor.getLatitude() + "&dropoff[longitude]=" + mDoctor.getLongitude();
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(uri));
                getContext().startActivity(intent);
            }
        });
        ...
        cvMaps.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String uri = "http://maps.google.com/maps?q=loc:" + mDoctor.getLatitude() + "," + mDoctor.getLongitude() + " (" + mDoctor.getName() + ")";
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(uri));
                getContext().startActivity(intent);
        ...
        placename.setText(mDoctor.getName());
        distance.setText(mDoctor.getDistance());
        mapdis.setText(mDoctor.getDistance());
        String url = "https://foursquare.com/v/" + mDoctor.getFrq_id();
        description.setText(url);

        Picasso.get().load(mDoctor.getImage()).into(ivbackcoll);
