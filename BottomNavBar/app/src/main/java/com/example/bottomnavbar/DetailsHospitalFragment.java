...
import com.example.bottomnavbar.databinding.HospitalFragmentDetailsBinding;
...
public class DetailsHospitalFragment extends Fragment {
    ...
    public DetailsHospitalFragment() {
        // Required empty public constructor
    }


    public static DetailsHospitalFragment newInstance(Hospital hospital) {
        DetailsHospitalFragment fragment = new DetailsHospitalFragment();
        Bundle args = new Bundle();
        args.putSerializable(ARG_PARAM_CONTACT, hospital);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            mHospital = (Hospital) getArguments().getSerializable(ARG_PARAM_CONTACT);
        }
    }

    HospitalFragmentDetailsBinding binding;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = HospitalFragmentDetailsBinding.inflate(inflater, container, false);

        view = inflater.inflate(R.layout.hospital_fragment_details, container, false);
        ...
        uber.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String uri = "https://m.uber.com/?client_id=<mHClirme28wuZEzqPiTuviNhVKd-FTtE>&dropoff[latitude]=" + mHospital.getLatitude() + "&dropoff[longitude]=" + mHospital.getLongitude();
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(uri));
                getContext().startActivity(intent);
            }
        });
        ...
        cvMaps.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String uri = "http://maps.google.com/maps?q=loc:" + mHospital.getLatitude() + "," + mHospital.getLongitude() + " (" + mHospital.getName() + ")";
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(uri));
                getContext().startActivity(intent);
        ...
        placename.setText(mHospital.getName());
        distance.setText(mHospital.getDistance());
        mapdis.setText(mHospital.getDistance());
        String url = "https://foursquare.com/v/" + mHospital.getFrq_id();
        description.setText(url);

        Picasso.get().load(mHospital.getImage()).into(ivbackcoll);
